...
class TntHelper
{
    public static function generateModuleNames($module_name, $icon){
        ...
        $module_name = Str::replace(" ","_",$module_name);
       
        $array['module'] = Str::ucfirst(Str::plural($module_name));
        $array['label'] = Str::ucfirst(Str::plural($module_name));
        $array['table'] = Str::lower(Str::plural($module_name));
        $array['model'] = Str::ucfirst(Str::singular($module_name));
        $array['icon'] = $icon;
        $array['controller'] = $array['model']."Controller";
        $array['singular_l'] = Str::lower(Str::singular($module_name));
        $array['singular_c'] = Str::ucfirst(Str::singular($module_name));
        return (object) $array;
        // var_dump($array);
    } 
    public static function getDBTables($remove_tables = []){
		// select de todas las tablas y se guarda en $tables
		// [{"Tables_in_la9sail2023":"abilities"}....
		...
		//tomamos el resultado del select y guardamos solo los nombres de las tablas
		...
		//listamnos todas las tablas que no queremos que se muestren
		...
		//tomamos el nombre de la tabla que ingrsa por la funcion y la unimos con 
		//el arreglo que listamos 
		$remove_tables = array_merge($remove_tables, $remove_tables2);
		//Toma un array y devuelve un nuevo array sin valores duplicados con array_unique
		$remove_tables = array_unique($remove_tables);
		//Compara array1 con uno o más arrays y devuelve los valores de array1 que no
		// estén presentes en ninguno de los otros arrays array_diff
    ...
    public static function getModuleNames($remove_modules = []) {
        $modules = Module::all();
		//probar con la tabla modullos llena
		$modules_out = array();
	...
	public static function print_menu($menu, $active = false) {
		...
		if(count($childrens)) {
			...
			foreach($childrens as $children) {
				$str .= TntHelper::print_menu($children);
			}
			$str .= '</ul>';
		}
		$str .= '</li>';
	...
	public static function setenv($param, $value) {
		...
		$envfile = TntHelper::openFile('.env');
		$line = TntHelper::getLineWithString('.env', $param.'=');
		$envfile = Str::replace($line, $param . "=" . $value."\n", $envfile);
